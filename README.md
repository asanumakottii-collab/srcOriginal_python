...
├── galaxy/        # 天の川専用原盤
└── unit_position/ # ユニット配置画像（設定時のみ）
```

印刷用 PDF は幅 191 mm × 高さ 277 mm の1ページ・ベクトルデータです。`-PDF` 使用時も「星座を処理しますか。(y/N)」と確認し、`y` を選ぶと星座名と星座線をPDFにベクトル出力します。SVG の向きマークはPDFには含みません。印刷時は「用紙に合わせる」を無効にし、100%(実寸)で出力してください。旧 `-PS` オプションは廃止され、使用するとエラーになります。
