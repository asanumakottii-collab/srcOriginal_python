...
自作の投影機式プラネタリウム(正十二面体などにレンズユニットを複数配置して天球全体を投影するタイプ)を作るとき、各投影機ユニットに取り付ける「原板」に星の穴をどこに開けるかを計算し、SVG または PostScript として出力します。
...
3. `PlateWriterSVG` / `PlateWriterPS` が変換結果を版下データとして書き出す
...
| `plate_writer.py` | SVG / PostScript 出力 |
...
依存ライブラリは `numpy`、`Pillow`(ユニット配置図の出力に使用)のみです。
...
# PostScript形式で出力する場合
python3 transformer.py -PS -f starconfig.properties
...
- `-PS` — 原板データを PostScript 形式で出力する(未指定時は SVG)
...
生成した SVG / PostScript とユニット配置画像は、設定ファイルの `output.directory` で指定したフォルダに出力されます。未指定時は `output` フォルダが自動作成されます。
...
ポリゴンは各原盤座標から天球へ光線を逆投影し、通常の星配置と同じユニット選択規則で境界を二分探索して求めます。枠の円でクリップ済みで、星原盤とは別のSVGに同じ用紙配置・黒い原盤・白い投影領域で出力されます。PostScriptで星を出力する場合も、ポリゴンはSVGです。
